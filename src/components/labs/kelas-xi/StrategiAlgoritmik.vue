<template>
  <div class="lab-container">
    <div class="lab-header">
      <h2>Strategi Algoritmik dan Pemrograman</h2>
      <p>Materi ini mengintegrasikan Berpikir Komputasional (rekursi, algoritma greedy, pemrograman dinamis), algoritma, dan pemrograman menggunakan bahasa C++.</p>
    </div>
    <div class="lab-workspace">
      <div class="placeholder-content">
        <h3>Materi Sedang Disusun</h3>
        <p>Halaman ini disiapkan untuk menampung modul materi Strategi Algoritmik dan Pemrograman.</p>
      </div>
    </div>
  </div>
</template>

<script setup>
</script>

<style scoped>
.placeholder-content {
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  height: 100%; text-align: center; color: var(--text-muted);
}
.placeholder-content h3 { font-size: 1.5rem; margin-bottom: 10px; color: var(--text-secondary); }
</style>
